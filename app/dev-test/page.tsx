  ...
  try {
    ...
    if (projects.rows.length === 0) {
      ...
      const newProjects = await db.query(
        sql`SELECT * FROM projects WHERE owner_id = 'dev-test-user' ORDER BY created_at DESC LIMIT 1`
      )

      if (newProjects.rows[0]) {
        redirect(`/projects/${newProjects.rows[0].id}`)
      }
    } else {
      // Redirect to first project
      redirect(`/projects/${projects.rows[0].id}`)
    }
  } catch (error) {
